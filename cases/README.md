...
shows a Jump List menu with `b64d`/`b64e`/`background`, plus a `json` category
(Pretty-print/Minify, running `json-pretty`/`json-pretty -m`) - hover any item
to see the command it runs. Each opens a terminal window that shows the
result and closes itself after 60 seconds.
